...
<div class="flex-row-13 ai-center product-card" data-offer="{{ $item['offer']['guid'] }}"
    data-variant="{{ $item['variant']['guid'] ?? '' }}"
    data-price="{{ $item['variant'] ? $item['variant']['price'] : $item['offer']['maxPrice'] }}"
    ...
    <a href="{{ route('offers.show', $item['offer']['guid']) }}" onclick="showPreloader()"
        ...
        @else
            <img class="lock"
                src="{{ route('images.proxy', ['type' => 'extension', 'guid' => $item['offer']['imageGuid']]) }}"
